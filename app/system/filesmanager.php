...
if(isset($get['exit']) && isset($_SESSION['template'])):
    unset($_SESSION['template']);
    redirect('app/system/dashboard.php');
endif;

if(isset($get['tpl']) && in_array($get['tpl'], array_diff(scandir(PATH_TPL), array('..', '.')))):
    $_SESSION['template']=$get['tpl'];
    redirect('app/system/filesmanager.php');
endif;

if(!in_array("files", $userApp['permissions']) || !in_array("site", $userApp['permissions']) || !in_array("router", $userApp['permissions']) || !in_array("mysql", $userApp['permissions']) || !isset($_SESSION['template']))
  ...
  $root_url = 'app'; //templates default??

  // Root path for file manager
  $root_path = PATH_TPL . $_SESSION['template'];//'default';//$_SERVER['DOCUMENT_ROOT'] . '/' . $root_url; //templates default ??
  ...
  <tr>
  <?php if (!FM_READONLY): ?><td><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></td><?php endif; ?>
  <td><div class="filename"><a href="<?php echo $filelink ?>" title="<?php echo fm_t('File info', $lang) ?>"><i class="<?php echo $img ?>"></i> <?php echo fm_convert_win($f) ?></a><?php echo ($is_link ? ' &rarr; <i>' . readlink($path . '/' . $f) . '</i>' : '') ?></div></td>
  ...
  <div class="path main-nav">
          ...
          <?php
          $path = fm_clean_path($path);
          $root_url = "<a href='filesmanager.php?exit=true'><i class='fa fa-home' aria-hidden='true' title='Dashboard'></i></a> <i class='fa fa-caret-right'></i> <a href='?p='><i class='fa fa-folder' aria-hidden='true' title='" . FM_ROOT_PATH . "'></i> ". $_SESSION['template'] ." </a>";
